                ...
                .then(response => {
                  if(response.ok){
                    response.json().then(data => {
                      dispatch(fetchProyectoresSuccess(data.proyectors,data.message));
                    })
                  }
                  else{
                    response.json().then(error => {
...
export const fetchProyectoresRequest = () => {
  return {
    type:'FETCH_PROYECTORS_REQUEST'
  }
}


//Sync action
export const fetchProyectoresSuccess = (proyectors,message) => {
  return {
    type: 'FETCH_PROYECTORS_SUCCESS',
